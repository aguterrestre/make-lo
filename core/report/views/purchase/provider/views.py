    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if action == 'search_report':
                ...
                # Filtro la busqueda de provider por fecha de creacion
                ...
                # Filtro la busqueda de provider por fecha de nacimiento
                if len(start_date) and len(end_date):
                    search = search.filter(date_birthday__range=[start_date, end_date])
                # Filtro la busqueda de provider por provider elegido
                if provider > 0:
